...
describe("tests for crud", () => {
  ...
  it("adds a task to a category", async () => {
    ...
    waitFor(() =>
      expect(
        result.current.category.find((cat) => cat.id === 1).tasks
      ).toHaveLength(1)
    );
  ...
  it("deletes a category", async () => {
    ...
    waitFor(() =>
      expect(
        result.current.category.find((cat) => cat.id === 1)
      ).toBeUndefined()
    );
    waitFor(() =>
  ...
  it("edits a category name", async () => {
    ...
    waitFor(() =>
      expect(result.current.category.find((cat) => cat.id === 1).cate).toBe(
        "Updated Category"
      )
    );
    waitFor(() =>
      expect(dbCrud.updateCategoryName).toHaveBeenCalledWith(
        "categories",
        1,
        "Updated Category"
      )
  ...
  it("edits a task", async () => {
    ...
    expect(dbCrud.updateTaskName).toHaveBeenCalledWith(
      "categories",
      1,
      2,
      "Updated Task"
